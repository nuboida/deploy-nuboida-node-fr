...
import { LogIn, Home, Lock, Mail, Eye, EyeOff } from 'lucide-react';
...
export function ResidentSignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.title && response.message) {
        localStorage.setItem('signin_toast_renter', JSON.stringify({
      ...
      <div className="login-card">
        ...
        <form onSubmit={handleSubmit} className="d-flex flex-column gap-3">
          ...
          <div className="form-check">
            <input
              type="checkbox"
              id="rememberMeResident"
              checked={rememberMe}
              onChange={(e) => setRememberMe(e.target.checked)}
              className="form-check-input"
            />
            <label htmlFor="rememberMeResident" className="form-check-label">
        ...
        <div className="d-flex justify-content-between align-items-start">
          <div className="d-flex flex-column align-items-start">
            ...
            <button
              type="button"
              onClick={() => navigate('/resident/forgot-password')}
